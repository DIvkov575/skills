...
One procedure, blank page → go/no-go. Two phases: **generate** (diverge), then **narrow** (converge by cheaply trying things). Core rule: effort ∝ 1/confidence — the cheapest experiment that could kill an idea runs first. Killing ideas early and cheap is the goal, not a failure.
...
**1c. Generate directions — wide, written directly.** Just write down ≥20 candidate directions yourself (no subagents). Push for breadth across angles so the span is wide: gaps in theory/bounds, inverted assumptions, techniques ported from adjacent fields, deployment/scale failure modes, abandoned ideas newer tools now make feasible. Each = one line: `"<First-to-X / X-beats-Y-because-Z>" — 2-sentence why-novel-and-winnable`. Bias for breadth and weirdness; a bad candidate costs ~nothing because the gate kills it cheaply.

**1d. Dedup → record the FULL peak set to the ledger.** Merge near-duplicates (keep sharpest phrasing). **No pruning, no ranking by quality.** Then **write every candidate to the ledger** (below) — the peak, captured before any Phase-2 attrition, so a failed attempt returns to the shelf instead of regenerating from scratch.
...
- Metric before method; pre-registered gates are immutable; >10–15% first-run gain = bug until audited. (Past projects faked 39–47% "wins" by violating these.)
- Surface negatives plainly; don't soften a kill to keep a project alive.
- Default to the cheaper test, not the bigger build. Building never fixes a selection problem.
- The ledger is the source of truth for what's been tried. Read it before generating; update on every status change. A failed attempt returns to the shelf, not a blank page.
